...
跨标签页协调（`auth-tab-channel.ts`）补三样东西：
...
export function createAdminHttpContext(axiosDefaults = {}, tabChannel?) {
...
const productionContext = createAdminHttpContext(
  ...
  import.meta.env.MODE === "test"
    ? createNoopAuthTabChannel() // 测试 / SSR：无锁无广播
    : createBrowserAuthTabChannel(() => authSessionStore.getSession(), {
...
src/api/http/          可复用套件（零项目依赖）
  ...
  ├── auth-tab-channel.ts         步骤 6：Web Locks + BroadcastChannel 跨标签页协调
...
5. 需要多标签页协调时传 `createBrowserAuthTabChannel(readSession, { channelName, lockName })`，每个项目用独立的频道名和锁名；测试或 SSR 传 `createNoopAuthTabChannel()`。
...
| `src/api/http/auth-tab-channel.spec.ts`        | 标签页存在感握手、单页免等待、peer 离场、事件广播不回投     |
