...
如果希望从 User / Admin / Super 登录开始，沿 Pinia、侧边栏、路由、按钮、接口和数据范围完整追踪，请先阅读 [RBAC 全链路教学案例](../router/rbac-end-to-end-flow.md)。
...
- **指令没有做响应式更新**，权限变了按钮不会自己长回来。当前指令只有 `mounted`：重新登录、F5 或路由离开再回来导致组件重新挂载时会按最新快照重建；同一页面内静默刷新令牌只会替换 Pinia 用户快照，现有按钮不保证立即同步（步骤 12 展开）。
...
- **前端快照：下次登录 / 刷新令牌 / profile 后更新。** `user.permissions` 是这些响应下发的快照。Pinia 快照更新后，侧边栏 `computed` 会自动重算，路由守卫会在下一次导航时使用新值；`v-permission` 只有 `mounted`，因此按钮要等组件重新挂载才完整同步。

这个组合的含义：权限**回收**在安全上即时兑现（接口立刻 403），界面可能短暂显示一个"点了会失败"的按钮；权限**授予**要先等快照更新，再等页面组件重新挂载，按钮才出现。当前没有为权限变化增加实时推送或响应式权限组件，这是步骤 1 里指令不做响应式更新的实际边界。

**动手验证**：给 admin 角色勾上「删除用户」按钮节点保存，用已登录的 admin 账号立刻 curl 删除接口（已放行），但页面按钮还没出现；刷新会话使 Pinia 获得新快照后，再重新进入用户页或 F5，让组件重新挂载，按钮才出现。
